...
magenta_authenticate = {'terminalid': '00:00:00:00:00:00', 'mac': '00:00:00:00:00:00', 'terminaltype': 'WEBTV',
                        'utcEnable': '1', 'timezone': 'UTC', 'userType': '3', 'terminalvendor': 'Unknown',
                        'preSharedKeyID': 'PC01P00002', 'cnonce': '5c6ff0b9e4e5efb1498e7eaa8f54d9fb'}
magenta_get_chlist = {'properties': [{'name': 'logicalChannel',
                                      'include': '/channellist/logicalChannel/contentId,/channellist/logicalChannel/name,/channellist/logicalChannel/pictures/picture/imageType,/channellist/logicalChannel/pictures/picture/href'}],
...
def magenta_get_channellist():
    ...
    magenta_chlist_url = session.post(magenta_channellist_url, data=json.dumps(magenta_get_chlist),
                                      headers=magenta_header)
...
def magenta_select_channels():
    ...
    if user_select is not None:
        ...
        if os.path.isfile(magenta_chlist_selected):
            valid = magenta_check_selected_list()
            if valid is True:
        ...
        magenta_check_selected_list()
        ok = dialog.ok(provider, 'Channellist unchanged!')

def magenta_check_selected_list():
    check = 'invalid'
...
def download_broadcastfiles():
    ...
    for user_item in selected_list['channellist']:
        ...
        magenta_data = {'channelid': channel, 'type': '2', 'offset': '0', 'count': '-1', 'isFillProgram': '1',
                        'properties': '[{"name":"playbill","include":"ratingForeignsn,id,channelid,name,subName,starttime,endtime,cast,casts,country,producedate,ratingid,pictures,type,introduce,foreignsn,seriesID,genres,subNum,seasonNum"}]',
                        'endtime': endtime + '235959', 'begintime': starttime + '000000'}
        ...
        pDialog.update(percent_completed, 'Downloading Broadcast Files for ' + user_item['name'] + ' ' + provider,
...
def create_magenta_xml_broadcast(enable_rating_mapper):
    ...
    for user_item in selected_list['channellist']:
        ...
        try:
            ...
            log(provider + ' no Programminformation for Channel ' + user_item['name'] + ' with ID ' + user_item[
                'contentId'] + ' avaivible')
    pDialog.close()
